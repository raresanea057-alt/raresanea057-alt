[README (1).md](https://github.com/user-attachments/files/28672279/README.1.md)
<div align="center">

# Rares Anea

**11th grade · Tulcea, Romania**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-connect-0A66C2?style=flat&logo=linkedin)](https://www.linkedin.com/in/rares-anea-73a246388/)
[![GitHub](https://img.shields.io/badge/GitHub-raresanea057--alt-181717?style=flat&logo=github)](https://github.com/raresanea057-alt)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)

*Competitive AI/ML · NLP Research · Olympiad contestant*

</div>

---

## 🏆 Competitions

| Competition | Scope | Result |
|---|---|---|
| **[ROAI 2026 — Selection Camp Qualifier (IX-XII)](https://judge.nitro-ai.org/competitions/roai-2025/baraj-nationala-2026/leaderboard/complete?participant_name=AneaRares&page=1&page_size=100)** | 🇷🇴 Romania | **Top 10 nationally** |
| **[ROAI 2026 — Selection Camp GPU Round (IX-XII)](https://judge.nitro-ai.org/competitions/roai-2025/lot-1-2026)** | 🇷🇴 Romania · invite-only | **Top 13 nationally** |
| **[ROAI 2026 — Selection Camp CPU Round (IX-XII)](https://judge.nitro-ai.org/competitions/roai-2025/lot-2-2026)** | 🇷🇴 Romania · invite-only | **Top 15 nationally** |
| **[ROAI 2026 — National Stage (XI-XII)](https://judge.nitro-ai.org/competitions/roai-2025/nationala-xi-xii-2026/leaderboard/complete?participant_name=AneaRares&page=1&page_size=100)** | 🇷🇴 Romania · invite-only | **Top 20 nationally** |
| **[ONIA 2026 — National Stage (XI-XII)]([https://judge.nitro-ai.org/competitions/roai-2025/nationala-xi-xii-2026/leaderboard/complete?participant_name=AneaRares&page=1&page_size=100](https://platform.olimpiada-ai.ro/ro/competitions/18?tab=final))** | 🇷🇴 Romania · invite-only | **Top 30 nationally** |


---

## 🔬 Projects

### [HistRoNER](https://github.com/raresanea057-alt/HistRoNER) — Historical Romanian NER Benchmark

> Extends [HistNERo (Avram et al., 2024)](https://huggingface.co/datasets/avramandrei/histnero) with a BiLSTM-CRF model + rule-based post-processing, benchmarked against Romanian BERT on 5 entity types in historical text.

| Model | Params | F1 |
|---|---|---|
| BiLSTM-CRF + casing + rules *(ours)* | ~2M | **0.44** |
| Romanian BERT *(Avram et al., 2024)* | 125M | 0.66 |

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat&logo=huggingface&logoColor=black)

---

## 🛠 Skills

**ML/DL** — BiLSTM · CRF · Transformers · CNNs · Reinforcement Learning  
**NLP** — NER · Sequence Labeling · Text Classification  
**CV** — Image Segmentation · Object Detection  
**Languages** — Python · C++ · Romanian · English
